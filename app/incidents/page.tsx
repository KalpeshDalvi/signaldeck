...
export default async function IncidentsPage() {
  ...
  const deploymentFinding = correlatedDeployment?.minutesBeforeFailure !== null && correlatedDeployment?.minutesBeforeFailure !== undefined && correlatedDeployment.minutesBeforeFailure >= 0
    ...
    : null;
  const likelyCause = deploymentFinding
  ...
  const telemetryTimeline = [
    ...deploymentEvidence.slice(0, 5).map((deployment) => ({
      time: deployment.record.observed_at,
      type: "Deployment",
      title: `${deployment.record.service_name} ${deployment.version}`,
      detail: [deployment.image, deployment.commitSha ? `commit ${deployment.commitSha.slice(0, 8)}` : null, deployment.actor ? `by ${deployment.actor}` : null].filter(Boolean).join(" · ") || deployment.record.message,
      tone: "deployment",
    })),
  ...
  return <>
    <PageHeader eyebrow="INCIDENT COMMAND" title={incident ? incident.title : "No active incident"} description="Correlate failures, changes, response activity, and the operational record.">
    ...
    <section className="incident-hero">
      <div>
        ...
        <p>{incident ? incident.summary : "Continue ingesting traces, logs, deployment changes, and Kubernetes events to detect future incidents."}</p>
    ...
    <div className="incident-metrics">
      ...
      <article><span>Changes</span><strong>{deploymentEvidence.length}</strong><small>{correlatedDeployment ? `${correlatedDeployment.confidence}% correlation confidence` : "No deployment evidence"}</small></article>
    </div>

    {correlatedDeployment ? <article className="panel change-evidence">
      <div className="panel-title"><div><h2>What changed?</h2><p>Deployment evidence correlated with the current failure window</p></div><span className="pill warning">{correlatedDeployment.confidence}% confidence</span></div>
      <div className="change-grid">
        <div><small>Deployment</small><strong>{correlatedDeployment.version}</strong><span>{correlatedDeployment.previousVersion ? `from ${correlatedDeployment.previousVersion}` : "previous version unavailable"}</span></div>
        <div><small>Timing</small><strong>{correlatedDeployment.minutesBeforeFailure !== null ? `${correlatedDeployment.minutesBeforeFailure} min before failure` : "No failure timing"}</strong><span>{new Date(correlatedDeployment.record.observed_at).toLocaleString()}</span></div>
        <div><small>Error-rate change</small><strong className={correlatedDeployment.errorRateDelta > 0 ? "bad-text" : "good-text"}>{signed(correlatedDeployment.errorRateDelta, "%")}</strong><span>{correlatedDeployment.before.errorRate.toFixed(1)}% → {correlatedDeployment.after.errorRate.toFixed(1)}%</span></div>
        <div><small>P95 change</small><strong className={correlatedDeployment.p95Delta > 0 ? "bad-text" : "good-text"}>{signed(correlatedDeployment.p95Delta, " ms")}</strong><span>{formatDuration(correlatedDeployment.before.p95)} → {formatDuration(correlatedDeployment.after.p95)}</span></div>
      </div>
      <div className="change-meta"><span>Image: {correlatedDeployment.image ?? "not supplied"}</span><span>Commit: {correlatedDeployment.commitSha ?? "not supplied"}</span><span>Actor: {correlatedDeployment.actor ?? "not supplied"}</span></div>
    </article> : null}

    <div className="incident-layout">
      <article className="panel incident-timeline">
        <div className="panel-title"><div><h2>Investigation timeline</h2><p>Deployment changes, telemetry evidence, and human decisions</p></div><span className="pill info">Live record</span></div>
        {timeline.length ? timeline.map((item, index) => <div className="timeline-item" key={`${item.time}-${index}`}><div className={`timeline-marker ${item.tone}`} /><time>{new Date(item.time).toLocaleTimeString()}</time><div><small>{item.type}</small><strong>{item.title}</strong><p>{item.detail}</p></div></div>) : <div className="empty-state"><h2>No incident evidence yet</h2><p>Failed traces, logs, deployment changes, and Kubernetes events will appear here automatically.</p></div>}
      </article>

      <aside className="incident-side">
        <article className="panel finding-card"><div className="panel-title"><div><h2>Current finding</h2><p>Correlation is a hypothesis until responders confirm it</p></div></div><strong>{likelyCause}</strong><ul><li>{primary ? `${primary.name}: ${primary.errorRate.toFixed(1)}% errors, P95 ${formatDuration(primary.p95)}` : "No affected service summary"}</li><li>{deploymentEvidence.length ? `${deploymentEvidence.length} deployment change record(s) are available.` : "No deployment evidence has been ingested."}</li><li>{k8sEvents.length ? `${k8sEvents.length} Kubernetes events may explain infrastructure impact.` : "No Kubernetes event evidence has been ingested."}</li></ul></article>
        <article className="panel action-card"><div className="panel-title"><div><h2>Next actions</h2><p>Recommended investigation sequence</p></div></div>{primary ? <><Link className="response-action" href={`/services/${encodeURIComponent(primary.name)}?environment=${encodeURIComponent(primary.environment)}`}><span>1</span><div><strong>Inspect affected service</strong><small>Review traces and correlated logs</small></div></Link><Link className="response-action" href="/traces"><span>2</span><div><strong>Compare failed traces</strong><small>Find the common slow dependency</small></div></Link><Link className="response-action" href="/kubernetes"><span>3</span><div><strong>Validate deployment impact</strong><small>Check rollout, pods, restarts, and events</small></div></Link></> : <Link className="response-action" href="/connect"><span>1</span><div><strong>Connect telemetry</strong><small>Deploy the collector to begin detection</small></div></Link>}</article>
      </aside>
    </div>
  </>;
}
